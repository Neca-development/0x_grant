...
const TokenCard: FunctionComponent<ITokenCardProps> = (props) => {
  ...
  const contractAddress = useMemo(
    () =>
      `${data.contractAddress?.substring(0, 4)}...${data.contractAddress?.substring(
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <div className="text-dark-gray font-semibold text-sm mb-1">{data.tokenId}</div>
        ...
        <button className="text-blue font-bold" id="pick-to-swap-button">
          Pick to swap
        </button>
      </div>
    </article>
