...
const OrderCard: FunctionComponent<ITokenCardProps> = (props) => {
  ...
  const contractAddress = useMemo(
    () =>
      `${data.collectionAddress?.substring(0, 4)}...${data.collectionAddress?.substring(
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <div className="text-dark-gray font-semibold text-sm mb-1">{data.id}</div>
        <div className="text-dark-gray font-semibold mb-3">{data.collectionName}</div>
        <div className="text-dark-gray font-semibold mb-3">{contractAddress}</div>
        <div className="text-dark-gray font-medium mb-4">
          Swap to: <span className="font-semibold">{data.tokenSymbol}</span>
        </div>
        <button className="text-blue font-bold" id="offer-overview-button">
          Offer overview
        </button>
      </div>
    </article>
